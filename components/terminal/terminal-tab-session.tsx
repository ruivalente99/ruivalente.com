...
import { TERM_THEMES, getTheme } from "@/lib/terminal/themes";
...
export interface TerminalTabSessionProps {
  tabId: string;
  fsRef: React.RefObject<VDir>;
  isActive: boolean;
  themeName: string;
  initialCwd?: string;
...
export function TerminalTabSession({
  tabId,
  fsRef,
  isActive,
  themeName,
  ...
  const [cwd, _setCwd] = useState(initialCwd);
  ...
  const setCwd = useCallback(
    (p: string) => {
      cwdRef.current = p;
      _setCwd(p);
      onTitleChange(tabId, `rsh: ${displayPath(p)}`);
    },
    [tabId, onTitleChange]
  );
  ...
  useEffect(() => {
    ...
    print(
      <pre className="t-acc text-[10px] sm:text-xs leading-tight whitespace-pre overflow-x-auto select-none">
        {BANNER}
      </pre>
    );
    println("");
    ...
    println(`${profile.name.toLowerCase()} · ${profile.role.toLowerCase()} · ${profile.bio.toLowerCase()}`, "t-dim lowercase");
    println("");
    print(
      <div className="t-dim lowercase">
    ...
    onTitleChange(tabId, `rsh: ${displayPath(initialCwd)}`);
  ...
  // refocus when returning to shell or tab becomes active
  useEffect(() => {
    if (isActive && view.kind === "shell") {
  ...
  const api = useMemo<TermAPI>(() => {
    ...
    return {
      ...
      openVim: (absPath: string) => {
        ...
        onTitleChange(tabId, `vim: ${name.toLowerCase()}`);
        setView({
          kind: "vim",
          path: absPath,
          name,
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [fsRef, setCwd, print, println, printLines, clear, setUser, themeName, onThemeChange, tabId, onTitleChange, router]);
  ...
  const handleTab = useCallback(() => {
    ...
    const apply = (cands: string[], partial: string, addSpace: boolean, display?: string[]) => {
      ...
      if (lcp.length > partial.length) {
        ...
        echoInput(val);
        print(
          <div className="flex flex-wrap gap-x-5 t-dim lowercase">
  ...
  }, [input, echoInput, print, fsRef]);

  // ---------- key handling ----------
  const onKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    // Intercept terminal shortcuts for new tab and close tab
  ...
  const rootClasses = [
    "term-root font-mono h-full w-full flex flex-col overflow-hidden select-text lowercase text-xs sm:text-sm",
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 min-h-0">
        ...
        {view.kind === "vim" && (
          <VimEditor
            ...
            onExit={() => {
              setView({ kind: "shell" });
              onTitleChange(tabId, `rsh: ${displayPath(cwdRef.current)}`);
              println("");
            }}
          />
        )}
      ...
      {/* Terminal hint / status bar */}
      <div
        className="shrink-0 px-3 py-1 border-t text-[10px] t-dim flex justify-between items-center gap-2 select-none"
        style={{ borderColor: "var(--t-selection)" }}
      >
        <span className="truncate">
          help · tab completes · ↑↓ history · games: {GAMES.join(" ")} · theme {theme.name}
        </span>
        <span className="hidden sm:inline shrink-0 font-mono">
          ruivalente.com · {user}@rui
        </span>
      </div>
    </div>
  );
